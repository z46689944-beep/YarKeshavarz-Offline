...
function exact(text,term){const a=normalize(text).split(" ").filter(Boolean),b=normalize(term).split(" ").filter(Boolean);if(!b.length)return false;if(b.length===1)return a.includes(b[0]);for(let i=0;i<=a.length-b.length;i++){let ok=true;for(let j=0;j<b.length;j++)if(a[i+j]!==b[j]){ok=false;break;}if(ok)return true;}return false;}
